...
namespace scales {
	...
	uint majorScale(uint root, uint at) {
		switch (at) {
	...
	uint minorScale(uint root, uint at) {
		return majorScale((root + 5) % 12, at); //relative major
	}

	uint chromaticScale(uint root, uint at) {
		return (root + at) % 12;
	}

	uint pentatonicMajorScale(uint root, uint at) {
		switch (at) {
		case 0: return root;				//W
		case 1: return (root + 2) % 12;		//W
	...
	uint pentatonicMinorScale(uint root, uint at) {
		return pentatonicMajorScale((root + 9) % 12, at);
	}

	uint bluesScale(uint root, uint at) {
